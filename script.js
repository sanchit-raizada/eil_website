...
const slideContents = [
    { title: 'Concept to <span class="accent">Commissioning</span><br>Total Energy Solutions', sub: "India's premier Engineering Consultancy & EPC company delivering world-class solutions across 27 countries." },
    { title: 'Engineering <span class="accent">Excellence</span><br>Since 1965', sub: "Six decades of trusted engineering consultancy for petroleum refineries, pipelines and petrochemical complexes." },
    { title: 'Global <span class="accent">Presence</span><br>Local Expertise', sub: "Significant footprints in MENA, Africa and Asia Pacific with an engineering hub in Abu Dhabi." },
    { title: 'Building a <span class="accent">Green</span><br>Energy Future', sub: "Leading India's energy transition with Green Hydrogen, Bio-fuels and sustainable engineering solutions." }
];
let cur = 0, autoTimer;
...
function goSlide(n) {
    ...
    setTimeout(() => {
        h1.innerHTML = ct.title; sub.textContent = ct.sub;
        h1.style.transition = 'all .6s ease'; sub.style.transition = 'all .6s ease .1s';
...
autoTimer = setTimeout(() => goSlide(1), 5000);
...
function scrollingParallex() {
    ...
    if (!el) return SyntaxError("No elemnt with class 'global-parallax' found");
    ...
    if (window.innerHeight > rect.top && rect.bottom > 0) {
        [...el.children].forEach(child => {
            child.style.transform = `translateY(${rect.top * 0.6}px)`;
        });
    }
}
